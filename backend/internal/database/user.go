...
func CreateUser(p *pgxpool.Pool, u *models.User) error {
	query := "INSERT INTO users(email,password_hash,is_verified) VALUES ($1,$2,$3) RETURNING id"
	err := p.QueryRow(context.Background(), query, u.Email, u.PasswordHash, u.IsVerified).Scan(&u.ID)
	if err != nil {
